...
export function DetailsCard({ disabled = false }: DetailsCardProps) {
  ...
  return (
    <div className="glass-card spaced">
      <div className="row clickable" onClick={() => !disabled && setDateModalOpen(true)}>
        <span className="emoji">{EMOJIS.CALENDAR}</span>
        <div className="row-content">
          <div className={`text-input subtle ${disabled ? 'disabled' : ''}`}>{event.dateTime || INPUT_PLACEHOLDERS.DATE_TIME}</div>
        </div>
      </div>
      <hr className="divider" />
      <div className="row clickable" onClick={() => !disabled && setLocationModalOpen(true)}>
        <span className="emoji">{EMOJIS.LOCATION}</span>
        <div className="row-content">
          <div className={`text-input subtle ${disabled ? 'disabled' : ''}`}>
            {event.location || <span className="text-input subtle">{INPUT_PLACEHOLDERS.LOCATION}</span>}
          </div>
        </div>
      </div>
      <hr className="divider" />
